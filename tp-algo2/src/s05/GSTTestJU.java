...
public class GSTTestJU {
	...
	// ------------------------------------------------------------
	static void rndAddRm(Random r, GeneralSearchTreeMap<Integer, Double> s, TreeMap<Integer, Double> u, int i) {
		if (r.nextBoolean()) {
	...
	// ------------------------------------------------------------
	static boolean areSetEqual(GeneralSearchTreeMap<Integer, Double> s, TreeMap<Integer, Double> u) {
		int lastKey = 10;
		if (!u.isEmpty())
			lastKey = u.lastKey();
	...
	public static void testRangeSearch(int n, int m) {
		...
		for (a = 0; a < n; a++) {
			...
			assertTrue("nbOfKeysInRange: " + sn + ", should be " + un + " (smallest elt)", sn == un);
			from = u.lastKey();
			to = from + 1;
			sn = s.nbOfKeysInRange(from, to);
			un = u.subMap(from, to).size();
