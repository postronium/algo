...
public class MaxIt {
    ...
    static class Board {
        ...
        public void undo() {
            ...
            crtCol = moves.peek().col();
            crtRow = moves.peek().row();
        }
    ...
    static int expectedScore(Board board, boolean isPlayerA,
        ...
        //in this case we try to maximise the score of A
        //      and minimise the score of B
        //the lower the score the best is the choice
        int bestScore = Integer.MAX_VALUE;
        Move bestMove = null;

        for (int i = 0; i < mt.length; i++) {
            board.play(mt[i]);
            int score = expectedScore(board, !isPlayerA, levels-1, decision);
            if (score < bestScore) {    //for both player the rational choice would be the lowest score (biggest value)
                bestScore = score;
                bestMove = mt[i];
            }
        ...
        decision.assign(bestMove);

        if (isPlayerA) {
            bestScore += board.grid[bestMove.col()][bestMove.row()];
        } else {
            bestScore -= board.grid[bestMove.col()][bestMove.row()];
        }
    ...
    static Move bestMoveAlphaBeta(Board board, boolean isPlayerA, int levels) {
        ...
        return bestMove(board, isPlayerA, levels);
    }
    ...
    // ------------------------------------------------------------
    private static Random rnd = new Random(125);
