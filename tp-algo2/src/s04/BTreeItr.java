...
public class BTreeItr<E> {
	...
	/**
	 * Goes down to the left as far as possible, from here. Always returns a bottom
	 * arc. Does not move this itr but creates a new one.
	 */
	...
	/**
	 * Goes down to the right as far as possible, from here. Always returns a bottom
	 * arc. Does not move this itr but creates a new one.
	 */
	...
	/**
	 * Replaces the subtree with that whole tree. PRE: !this.isInside(t). POST: t is
	 * now empty.
	 */
	...
	/**
	 * Moves the neighboring nodes according to a left rotation. PRE: ...
	 */
	public void rotateLeft() {
		BTree<E> rightBranche = this.right().cut();
		BTreeItr<E> rightBrItr = new BTreeItr<E>(rightBranche);
		this.right().paste(rightBrItr.left().cut());
		rightBrItr.left().paste(this.cut());
		this.paste(rightBranche);
	}

	/**
	 * Moves the neighboring nodes according to a right rotation. PRE: ...
	 */
	public void rotateRight() {
		BTree<E> leftBranche = this.left().cut();
		BTreeItr<E> leftBrItr = new BTreeItr<E>(leftBranche);
		this.left().paste(leftBrItr.right().cut());
		leftBrItr.right().paste(this.cut());
		this.paste(leftBranche);
	}
	...
	private boolean isConsistent() {
		...
		if (u != null && d != ((isLeftArc) ? (u.left) : (u.right)))
			return false;
		while (u != null) {
			if (d != null && d.parent != u)
				return false;
			if (d != u.left && d != u.right)
				return false;
			d = u;
			u = u.parent;
		}
