...
public class Ex2 {
	...
	// ------------------------------------------------------------
	static void rndAddRm(Random r, GSTMap<Integer, Double> s, TreeMap<Integer, Double> u, int i) {
		if (r.nextBoolean()) {
	...
	// ------------------------------------------------------------
	static boolean areSetEqual(GSTMap<Integer, Double> s, TreeMap<Integer, Double> u) {
		int lastKey = 10;
		if (!u.isEmpty())
			lastKey = u.lastKey();
	...
	static class GSTMap<K extends Comparable<K>, V> {
		...
		class GSTNode<K1 extends Comparable<K1>, V1> {
			...
			public V1 getVal(int i) {
				return vals.get(i);
			}

			public GSTNode<K1, V1> getSon(int i) {
				return sons.get(i);
			}
			...
			// ------------------------------------------------------------
			private int nbOfKeysOrSons() {
				return isLeaf ? nbOfKeys() : nbOfSons();
			}

			private boolean isFull() {
				return nbOfKeysOrSons() > maxNodeSize;
			}

			private boolean isLight() {
				return nbOfKeysOrSons() <= maxNodeSize / 2;
			}
