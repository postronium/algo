...
public class WeightedDiGraph {
  ...
  public WeightedDiGraph(int nbOfVertices){
    ...
    for (int i=0; i < size; i++) {
      outNeighbors.add(new HashMap<Integer,Integer>());
      inNeighbors.add (new HashMap<Integer,Integer>());
    }
  }
  // --------------------
  ...
  public SortedSet<Integer> neighboursFrom(int fromVid) {
    return new TreeSet<Integer>(outNeighbors.get(fromVid).keySet());
  }
  // --------------------
  public SortedSet<Integer> neighboursTo(int toVid) {
    return new TreeSet<Integer>(inNeighbors.get(toVid).keySet());
  }
  //======================================================================
  ...
  public List<Integer> pathBetween(int from, int to) {
    ...
    while(!fifo.isEmpty()) {
      ...
      for(int next:neighboursFrom(crt)) {
        LinkedList<Integer> augmList=new LinkedList<Integer>(list);
