...
public class FlowsTestJU {
  // ----------------------------------------------------------------------
  @Test public void testMaxFlow() {
    testIt(9, false);
  }
  
  @Test public void testMinCutAndThroughput() {
    testIt(9, true);
  }
  ...
  static int naiveMinCut(WeightedDiGraph g, int s, int t, List<Integer> sReach0) {
    List<Integer> sReach=new LinkedList<Integer>(sReach0);
    if (!sReach.contains(t)) return Integer.MAX_VALUE;
    List<Integer> allVert=new LinkedList<Integer>();
    ...
    possibleSubsets.add(new LinkedList<Integer>()); // for s alone
  ...
  static void select(int[] t, int[] sol, int m, int from, List<List<Integer>> res) {
    if (m==0) {
      List<Integer> crt=new LinkedList<Integer>();
